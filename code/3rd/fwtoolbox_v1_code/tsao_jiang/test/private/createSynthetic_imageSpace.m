...
%% Date last modified: August 19, 2011
%%
...
for ks=1:length(trueParams.species)
  ...
  try 
    freqs = gyro*fieldStrength*-algoParams.species(ks).frequency;  % frequency is in ppm, which is negated compared to precession frequency
  catch 
    freqs = 0;
  end
  ...
  for kt=1:N
    s(1,1,1,1,kt) = sum(relAmps(:).*exp(-j*2*pi*freqs(:)*t(kt))); % negated sign
    fieldAndR2starEffect(:,:,:,:,kt) = exp(-t(kt)*repmat(r2starmap,[1 1 1 C]) - j*2*pi*t(kt)*repmat(fieldmap,[1 1 1 C]));  % negated sign
  end
