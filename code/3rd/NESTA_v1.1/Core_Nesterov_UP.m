...
voxelSize_mm = setOpts('voxelSize_mm', [1, 1, 1]);
gradWeighting4D = setOpts('gradWeighting4D', 1);
...
for k = 0:maxiter-1,
   ...
   if strcmp(TypeMin,'tv')
       [df,fx] = Perform_TV_Constraint(xk, mu, U, Ut, voxelSize_mm, gradWeighting4D);
   end
...
function [df,fx] = Perform_TV_Constraint(xk, mu, U, Ut, voxelSize_mm, gradWeighting4D)
...
%     grad = compute_forward_difference_gradient(x, voxelSize_mm);
    grad = fgrad(x, voxelSize_mm);      % from medi toolbox
    if nargin == 6
        Dx = gradWeighting4D .* grad;
    else
        Dx = grad;
    end
    ...
    df = Ut(bdiv(u, voxelSize_mm));     % from medi toolbox
