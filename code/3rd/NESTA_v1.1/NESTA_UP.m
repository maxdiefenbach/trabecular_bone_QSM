...
function th=ValMUTv(x)
    ...
    % disp(sprintf('%d', size(Dv)))
    % disp(sprintf('%d', size(Dh)))
    ...
    % th = max(sk);
%     grad = compute_forward_difference_gradient(x);
    grad = fgrad(x);                    % use medi toolbox function 
    % size(grad)
    % plot_slice(grad(:, :, :, 1))
    % plot_slice(grad(:, :, :, 2))
    % plot_slice(grad(:, :, :, 3))
    
    th = max(sqrt(sum(abs(grad(:)).^2)));
end
